...
def load_dual_expert_system():
    try:
        ...
        pack_penalty = joblib.load('model_artifacts_v24.pkl')
        
        tabm_normal = pack_normal['models']['True TabM']
        # 对应 v24 保存时的键名
        tabm_penalty = pack_penalty['models']['True TabM (Dual-Regime v24)']
        
        X_cols_v24 = pack_penalty['X'].columns.tolist()
        X_cols_v62 = pack_normal['X'].columns.tolist()
        X_medians = pack_penalty['X'].median(numeric_only=True).to_dict()
        
        return X_cols_v24, X_cols_v62, X_medians, tabm_normal, tabm_penalty
    except Exception as e:
        st.error(f"严重错误：找不到内核文件。确保 v6_2 和 v24 的 pkl 文件在同级目录下。详细信息: {e}")
        st.stop()

X_cols_v24, X_cols_v62, X_medians, model_normal, model_penalty = load_dual_expert_system()
...
fg_cols = [c for c in X_cols_v24 if c.startswith('FG_')]
dom_cols = [c for c in X_cols_v24 if c.startswith('DOM_')]
log_handled = ['Log_specific surface area m2/g', 'Log_molecular weight', 'Log_adsorption time min', 'Log_C0_to_Dose_Ratio']
remaining_cols = [c for c in X_cols_v24 if c not in fg_cols and c not in dom_cols and c not in log_handled and c not in hidden_cols]
...
# 3. UI 界面布局
# ==========================================
st.set_page_config(page_title="Qm Predictor (v24 双态突变版)", layout="wide")
...
st.markdown("基于物理先验引导的混合专家模型 (Hard-Routing MoE)，集成 **v24 双态物理力场 (促进突变 vs 竞争深渊)** 的 Gated TabM 引擎。")
...
with tab_mat:
    ...
    with col1_m:
        if 'Log_specific surface area m2/g' in X_cols_v24:
    ...
    with col2_m:
        if 'Log_molecular weight' in X_cols_v24:
