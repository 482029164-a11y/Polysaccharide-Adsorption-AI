...
# 🚀 注入 __main__ 空间，这是 joblib 加载 pkl 的生命线
import __main__
...
def load_dual_expert_system():
    try:
        ...
        # 使用 v16 的列名作为全局准则（它包含了所有物理门控特征）
        X_cols_v16 = pack_penalty['X'].columns.tolist()
        X_cols_v62 = pack_normal['X'].columns.tolist()
        X_medians = pack_penalty['X'].median(numeric_only=True).to_dict()
        
        return X_cols_v16, X_cols_v62, X_medians, tabm_normal, tabm_penalty
    except Exception as e:
        st.error(f"严重错误：找不到内核文件。请确保 v6_2 和 v16 的 pkl 文件在同级目录下。详情: {e}")
        st.stop()

X_cols_v16, X_cols_v62, X_medians, model_normal, model_penalty = load_dual_expert_system()

def get_median(col_name):
    return float(X_medians.get(col_name, 0.0))

# 分类逻辑 (排除 Physical_Gate_Inhibition)
fg_cols = [c for c in X_cols_v16 if c.startswith('FG_')]
dom_cols = [c for c in X_cols_v16 if c.startswith('DOM_')]
log_handled = ['Log_specific surface area m2/g', 'Log_molecular weight', 'Log_adsorption time min', 'Log_C0_to_Dose_Ratio']
remaining_cols = [c for c in X_cols_v16 if c not in fg_cols and c not in dom_cols and c not in log_handled and c != 'Physical_Gate_Inhibition']
...
# 3. 界面层与交互设计 (保留用户原始风格)
# ==========================================
st.set_page_config(page_title="Qm Predictor (Dual-Routing v16)", layout="wide")
...
with tab_mat:
    ...
    with col1_m:
        if 'Log_specific surface area m2/g' in X_cols_v16:
    ...
    with col2_m:
        if 'Log_molecular weight' in X_cols_v16:
...
if st.button("运行计算", use_container_width=True):
    ...
    try:
        ...
        if c0_raw < 10.0 and ha_val > 0.0:
            st.warning(f"⚠️ 物理门控触发：检测到 C0 ({c0_raw:.2f} mg/L) 属于极低浓度区域且受到 HA 竞争干扰。底层已切换至【局部纠偏专家 True TabM (v16)】。")
            
            # v16 推理：必须注入物理门控特征
            user_inputs['Physical_Gate_Inhibition'] = 1.0
            final_df = pd.DataFrame([user_inputs]).reindex(columns=X_cols_v16)
            # 填补 NaN
            for col in X_cols_v16:
                if pd.isna(final_df[col][0]): final_df[col] = get_median(col)
            
            pred_log = model_penalty.predict(final_df)[0]
            engine_used = "v16 纠偏流形"
        else:
            st.success(f"✅ 物理门控触发：检测为常规浓度或纯水基质体系。底层已切换至【全局热力学专家 True TabM (v6.2)】。")
            
            # v6.2 推理：剔除 v16 特有的列，保持特征空间一致
        ...
        st.metric(label=f"理论吸附量 Qm (mg/g) [推理内核：{engine_used}]", value=f"{main_prediction:.4f}")
