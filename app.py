...
# ==========================================
# 1. 类定义 (与 XAI.py 完全统一，修复反序列化与设备冲突)
# ==========================================
...
class PyTorchDeepEnsembleRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        for m in self.models_: 
            ...
            m.eval()
        with torch.no_grad():
            return torch.cat([m(X_t) for m in self.models_], dim=1).mean(dim=1).numpy()
...
class PyTorchTrueTabMRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        X_t = torch.tensor(X.values if isinstance(X, pd.DataFrame) else X, dtype=torch.float32).to(device)
        with torch.no_grad():
            return self.model_(X_t).mean(dim=1).numpy()
