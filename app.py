...
def load_dual_expert_system():
    try:
        ...
        pack_penalty = joblib.load('model_artifacts_v23.pkl')
        
        tabm_normal = pack_normal['models']['True TabM']
        # 严格对应 v23 保存时的键名
        tabm_penalty = pack_penalty['models']['True TabM (Ultra v23)']
        
        X_cols_v23 = pack_penalty['X'].columns.tolist()
        X_cols_v62 = pack_normal['X'].columns.tolist()
        X_medians = pack_penalty['X'].median(numeric_only=True).to_dict()
        
        return X_cols_v23, X_cols_v62, X_medians, tabm_normal, tabm_penalty
    except Exception as e:
        st.error(f"严重错误：找不到内核文件。确保 v6_2 和 v23 的 pkl 文件在同级目录下。详细信息: {e}")
        st.stop()

X_cols_v23, X_cols_v62, X_medians, model_normal, model_penalty = load_dual_expert_system()
...
fg_cols = [c for c in X_cols_v23 if c.startswith('FG_')]
dom_cols = [c for c in X_cols_v23 if c.startswith('DOM_')]
log_handled = ['Log_specific surface area m2/g', 'Log_molecular weight', 'Log_adsorption time min', 'Log_C0_to_Dose_Ratio']
remaining_cols = [c for c in X_cols_v23 if c not in fg_cols and c not in dom_cols and c not in log_handled and c not in hidden_cols]
...
# 3. UI 界面布局
# ==========================================
st.set_page_config(page_title="Qm Predictor (v23 三次方重力坍缩版)", layout="wide")
...
st.markdown("基于物理先验引导的混合专家模型 (Hard-Routing MoE)，集成 **v23 三次方非线性重力下压** 的 Gated TabM 引擎。")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 引擎调度监控\n✅ 常规专家: True TabM (v6.2)\n✅ 纠偏专家: Gated TabM (v23)\n系统将自动推演 `Asymptotic Inhibition Force`，在极端竞争区间执行非线性重力坍缩预测。")
...
with tab_mat:
    ...
    with col1_m:
        if 'Log_specific surface area m2/g' in X_cols_v23:
    ...
    with col2_m:
        if 'Log_molecular weight' in X_cols_v23:
